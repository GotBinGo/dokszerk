...
{
	name=canvas,
	description={Vászon amire rajzolni lehet}
}
\newglossaryentry{javascript}
{
	name=JavaScript,
	description={Webes programozási nyelv}
}
\newglossaryentry{chrome}
{
	name=chrome,
	description={Googl által fejlesztett webböngésző}
}
\newglossaryentry{x86}
{
	name=x86,
	description={Elterjedt porcesszor utasításkészlet}
}
\newglossaryentry{websocket}
{
	name=WebSocket,
	description={HTTP szabványos, full-duplex kommunikációs megoldás}
}
...
\newacronym{wasd}{WASD}{A billentyűzet W, A, S és D gombjai, ami megfeleltethető a fel, balra, le, jobbra nyilaknak}
...
törekszik. A dizájn\index{dizájn} jellemzően néhány előre kiválasztott színnel
...
\Apageref{jatek}. oldalon kifejtetteket követve célom, hogy a játékot minél több felhasználó kipróbálhassa,
...
A böngészős\index{böngészős} alkalmazásunk programozásához több megoldás közül
...
\gls{javascript} futtatókörnyezet fejlődésével. Ez azt jelenti, hogy csakis
egy tetszőleges modern böngészőt például a Mozilla Firefox-ot vagy a
Google \Gls{chrome}-ot kell beszereznünk, ha még nem tettük. Ez tehát a
felhasználói oldal.
...
A \gls{javascript} nyelvben lehetőség van az objektumorientált programozásra.
...
A Node.js egy szerver-oldali \gls{javascript} alapú futtatókörnyezet. Ez
\Gls{chrome} V8 \gls{javascript} motorjára épül, amely már évek óta a legjobban
teljesítő \gls{javascript}-motor. Sikerét jórészt annak köszönheti, hogy a
...
A \gls{websocket} \cite{3} technológia használható gyors kétirányú adatküldésre. A
protokoll célja, a http korlátozott kérés-válasz struktúrájából származó
korlátozottságok és nagymennyiségű fejlécadat elkerülése. A \gls{websocket}
...
A \gls{websocket} szerver megadott \acrshort{tcp} porton hallgat és várja a
...
\gls{javascript}-el rajzolhatunk alakzatokat, így téglalapot, kört, szöveget
vagy akár raszter, sőt vektorgrafikus képeket is. A \gls{canvas} alapú
...
A felület egyszerű lesz. \Aref{vaz}. ábrán látható az alapvető elosztás, ami
...
Google \Gls{chrome}-ot is használok. Így biztosítani tudom, hogy mindkét
...
szabványos \gls{javascript}nek így a V8-nak sem szerves része.
...
böngésző által támogatott \gls{websocket}-re esett. Ez viszont nem minden. A
...
A parancsok mit sem érnek, ha nincsenek végrehajtva. A parancsértelmező\index{parancsértelmező}
...
A \gls{javascript} nyelvben lehetőség van jellemzők létrehozására, Ez viszont
...
A \gls{javascript} alapértelmezetten támogatja az időzítőket. A mellékelt kódban
...
a szerver, mind a kliens oldalán. De fontos, hogy a játék \aref{spec}. pontban említetteknek megfeleljen.
...
A játék működéséhez kulcsfontosságú az ütközések\index{ütközések} helyes kezelése. A
...
\Aref{kor:mozg}. ábrán a pontszerű ütközés számításának lépéseit lehet nyomon
...
objektum áll. Aref{kor:mozg}. ábrának az első lépésében láthatjuk, hogy az
...
mutat. Ezt \aref{kor:mozg}. ábra második lépése mutatja. Itt látszik a mozgó test
...
megmarad, áttevődik. Ahogyan \aref{kor:utk}. ábra is mutaja, ha egy 
...
síkra merőleges egyenesből. Ennek kiszámításához hasznos \aref{eq:group_first}. képletben látható
 atan2 függvény. A következő lépésben a kapott szöget kivonjuk az irányvektorból. Ez látható \aref{eq:group_second}. sorban.
...
A saját algoritmus hibáit \aref{eq:single}. képlet alapján javítottam, így már tökéletesen
...
részén jelenik meg a fő tartalom. \Aref{lobby}. ábrán látható, hogy az
...
\Aref{game}. ábrán látható maga a játék. A csapatok összesített pontszáma
...
A játék irányítása a nyíl gombokkal, vagy a \acrshort{wasd} gombokkal történhet. Az
...
programokat nem kell fordítani, ezért elég csak a forrásfájlokat \index{forrásfájlok}
...
C:\textbackslash{}Program Files (\gls{x86})\textbackslash{}nodejs mappában
...
Mac OS X \index{Mac OS X}rendszeren a telepítés nodejs.org/downloads-ról letölthető .pkg
...
A program minden részét \gls{javascript} nyelven írtam. Eleinte alig ismertem
a technológiát, a fejlesztés során viszont megbarátkoztam a számomra új
nyelvezettel. Mára mondhatom, hogy ismerem a \gls{javascript}-et és biztos
